...
export const OptionsModal: React.FC<{
  ...
  const defaultSmallPrinter =
    localStorage.getItem(LOCALSTORAGE_KEYS.smallPrinter) ?? "";
  const defaultBigPrinter =
    localStorage.getItem(LOCALSTORAGE_KEYS.bigPrinter) ?? "";

  const defaultHostName =
    localStorage.getItem(LOCALSTORAGE_KEYS.host) ?? currentHostName;
  const defaultPort =
    localStorage.getItem(LOCALSTORAGE_KEYS.port) ?? currentPort;

  const defualtDevMode =
    localStorage.getItem(LOCALSTORAGE_KEYS.devMode) ?? "false";

  const printerSelectOptions: React.ReactNode = printers.map(
    (printer, index) => (
  ...
  const updateHandler = () => {
    const smallPrinterValue = (smallPrinterRef.current as HTMLSelectElement)
      .value;
    ...
    if (hostValue === "") hostValue = currentHostName;
    ...
    >
      <div className={modalStyles["modal-container"]}>
        ...
        <select
          ...
          name="smallPrinter"
          id="smallPrinter"
          defaultValue={defaultSmallPrinter}
        >
        ...
        <select
          ...
          name="bigPrinter"
          id="bigPrinter"
          defaultValue={defaultBigPrinter}
        >
        ...
        <input
          type="button"
          onClick={factoryResetHandler}
          value="reiniciar de fabrica"
        />
        <input
          type="button"
          onClick={clearBitmapHandler}
          value="limpiar bitmap"
        />

        <h3>Funciones de desarrollador</h3>
        <label htmlFor="devMode">devMode</label>
        <select
          defaultValue={defualtDevMode}
          ref={devModeRef}
          name="devMode"
          id="devMode"
        >
